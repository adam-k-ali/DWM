...
Products (`dwm/docs/palettes/products.json`) choose archetype, template, and which palette is host / handle / mineral; a palette file itself is not typed as host or family. Gem/crystal/ingot/`block` products omit `host` and `handle`. Tool products (pickaxe/sword/shovel/axe/hoe) use `handle` + `mineral` (no `host`). Headless `--export-product` also supports `block` (iron-block layout → compact storage cubes).
...
# Headless product export (gem/crystal/ingot/block/ore/pickaxe/sword/shovel/axe/hoe):
...
#   --export-product silver_dalekanium_block --out dwm/src/client/resources/assets/dwm/textures/block/silver_dalekanium_block.png
#   --export-product bronze_dalekanium_block --out dwm/src/client/resources/assets/dwm/textures/block/bronze_dalekanium_block.png
