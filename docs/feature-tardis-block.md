...
- **Phase 3 portal pass:** renders the synced sky, Phase 2 terrain buffers, block entities, and entities into a full-window color/depth framebuffer from a fixed exterior door hitch looking outward (same stable lookout as the Phase 0/2 fallback), then composites that texture through the door aperture. Interior player movement does not dolly or pan the exterior eye.
...
- While ghost chunks/meshes are initially loading, the Phase 0/2 direct lookout remains as a temporary fallback with `applyLookoutStableView`. Phase 3 uses the same fixed hitch/outward eye via `SotoPortalCameraTransform` rather than interior-to-exterior parallax.
